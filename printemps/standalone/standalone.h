...
/*****************************************************************************/
#ifndef PRINTEMPS_STANDALONE_STANDALONE_H__
#define PRINTEMPS_STANDALONE_STANDALONE_H__
...
class Standalone {
   ...
   public:
    ...
    /*************************************************************************/
    Standalone(const int argc, const char *argv[]) {
        this->initialize();
    ...
    inline void setup(const int argc, const char *argv[]) {
        ...
        /**
         * Read the specified instance file and convert to the model.
         */

        if (m_argparser.instance_file_name.empty()) {
            throw std::runtime_error(printemps::utility::format_error_location(
                __FILE__, __LINE__, __func__,
                "No instance file is specified."));
        }
        const std::string EXTENSION = printemps::utility::extract_extension(
            m_argparser.instance_file_name);
        ...
        if (EXTENSION == "mps") {
            ...
            throw std::runtime_error(printemps::utility::format_error_location(
                __FILE__, __LINE__, __func__,
                "The specified instance file format is not supported."));
        }

        m_model.set_name(
            printemps::utility::base_name(m_argparser.instance_file_name));
    ...
    inline void solve(void) {
        ...
        if (m_argparser.include_instance_loading_time) {
